...
class PublicKeyUtils:
    ...
    def __init__(self, words, str_derivation_path="m/44'/637'/0'/0'/0'", curve=Ed25519, modifier=b'ed25519 seed'):
        self.privdev = 0x80000000
        self.curve = curve
        self.str_derivation_path = str_derivation_path
        self.modifier = modifier

        self.private_key = self.mnemonic_to_private_key(words)
        self.public_key = self.PublicKey25519(self.private_key)
    ...
    def derive_bip32childkey(self, parent_key, parent_chain_code, i):
        ...
        if (i & self.privdev) != 0:
            ...
            key = bytes(self.PublicKey25519(parent_key))
    ...
    def mnemonic_to_bip39seed(self, mnemonic, passphrase):
        ...
        salt = bytes("mnemonic" + passphrase, 'utf8')
